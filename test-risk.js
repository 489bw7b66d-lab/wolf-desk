...
export const tests = [
  ...
  ['15× ohne Stop, 4 % von 6 % übrig = Warnung', () => liqCheck({ side: 'long', liq: 96, leverage: 15 }, 100, null, 4, R, false).status === 'warn'],
  ['15× ohne Stop, nur noch 2 % = Verstoß (über die Hälfte verbraucht)', () => liqCheck({ side: 'long', liq: 98, leverage: 15 }, 100, null, 2, R, false).status === 'bad'],
  ['Buddys NEAR: 10× ohne Stop, 2,4 % von 9 % = Verstoß', () => liqCheck({ side: 'long', liq: 97.6, leverage: 10 }, 100, null, 2.4, R, false).status === 'bad'],
  ...
  // Live-Kurs gegenüber Plan
  ['Live: Kurs in der Zone', () => priceVsPlan({ dir: 'long', zone: [100, 102], stop: 95 }, 101).state === 'zone'],
  ['Live: Long über der Zone = hinterherlaufen, +2,94 %', () => { const r = priceVsPlan({ dir: 'long', zone: [100, 102], stop: 95 }, 105); return r.state === 'chasing' && near(r.distPct, (3 / 102) * 100); }],
  ['Live: Long unter Stop = ungültig', () => priceVsPlan({ dir: 'long', zone: [100, 102], stop: 95 }, 94).state === 'invalid'],
  ['Live: Short unter der Zone = hinterherlaufen', () => priceVsPlan({ dir: 'short', zone: [100, 102], stop: 106 }, 98).state === 'chasing'],
  ['Einstieg = Live-Kurs: R neu berechnet', () => { const p = withEntry({ dir: 'long', zone: [100, 102], stop: 95, entry: 101, tps: [110] }, 103); return p.R === 8 && p.liveEntry && p.tps[0] === 110; }],
  ['Einstieg hinter Stop nicht möglich', () => withEntry({ dir: 'long', zone: [100, 102], stop: 95 }, 94) === null],
  ['Hebel manuell: über Börsen-Maximum = Verstoß', () => leverageIssues(25, { exchangeMax: 20 }).some((i) => i.status === 'bad')],
