import { tradeHistory, openTradeFor, closedTrades, perfSplit, change24h, entryDistance, tradeStats } from './core-trades.js';
...
export const tests = [
  ...
  ['Statistik: Trefferquote, Ø Gewinn und Ø Verlust', () => {
    const t = [{ closedAt: 1, realized: 100, exits: [1, 2] }, { closedAt: 2, realized: -50, exits: [1] }, { closedAt: 3, realized: 50, exits: [1, 2, 3] }, { closedAt: 4, realized: -30, exits: [1] }];
    const s = tradeStats(t);
    return s.n === 4 && s.winRate === 50 && near(s.avgWin, 75) && near(s.avgLoss, -40) && near(s.payoff, 1.875);
  }],
  ['Statistik: Teilverkäufe gegen alles auf einmal', () => {
    const s = tradeStats([{ closedAt: 1, realized: 100, exits: [1, 2] }, { closedAt: 2, realized: -50, exits: [1] }]);
    return s.split.n === 1 && s.split.total === 100 && s.single.winRate === 0;
  }],
  ['Statistik: angeschnittene Trades zählen nicht', () => tradeStats([{ closedAt: 1, realized: 5, exits: [1], partial: true }]).n === 0],
